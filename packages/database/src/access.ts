...
export async function restrictBrowserAccess(db: SqlClient) {
  await db.query(`DO $$ DECLARE role_name text; table_name text; BEGIN
    FOREACH table_name IN ARRAY ARRAY['blocks','transactions','raw_events','transfers','address_entries','indexer_state','webhooks','webhook_deliveries','schema_migrations','validation_runs'] LOOP
