...
Five retention tests cover complete-block pruning, replay/restart continuity, rollback, unreclaimable storage protection, recovery of a durable pruning plan after a later batch fails, and pre-commit size guards for both ingestion and projection. Three additional atomic-ingestion tests cover replay, gap rejection, conflicting-evidence rollback and a simulated 300 MB database reading that rolls back both history and checkpoint. The complete unit/integration suite contains 75 tests. Seven browser scenarios cover the explorer plus landing copy, clipboard controls, API examples, FAQ keyboard interaction and reduced-motion mobile layout. Production Docker and local Next.js builds are checked independently. These tests do not certify Supabase quota behavior during a provider outage; that requires a successful live maintenance run and measured database size.

On 2026-09-22, the replacement Supabase project passed a live retention smoke: six Mainnet blocks were ingested, two oldest blocks pruned, physical compaction completed, the checkpoint preserved, replay remained idempotent, and the next block committed normally. Database size after the smoke was 12,266,643 bytes. Independent validation of retained blocks 22,124,979–22,124,982 checked 15 transactions and 24 canonical movements with zero fee/accounting mismatches. This is a bounded sample, not a full-chain audit or a quota stress test.
