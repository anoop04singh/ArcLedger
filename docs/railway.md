...
`WORKER_PAUSED=true` on indexer and ledger explicitly suspends their loops without database connections, including after GitHub-triggered redeploys. Set it to `false` and redeploy only after storage is healthy. Do not rely on removing all Railway regions: Railway may restore a default replica.
